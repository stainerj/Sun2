...
public class SunCalculator3 {
    ...
    public final static double ELOP = 102.9373;//ecliptic longitude of perihelion
    public static double rASun = 0;
    public static double decSun = 0;
    ...
    public final static double OOTE = 23.4393;//obliquity of the ecliptic
    ...
    public static double jRise = 0;
    public static double jSet = 0;
    ...
    public static void main(String[] args) {
        ...
        System.out.println("jdaytotime ---- " + julianDayToTime(jTransit, julianDayRaw));
    }
    ...
    //convert (gregorian) calender date to julian day number
    ...
    public static double dateTimeToJulianDay() {
        ...
        System.out.println("Date/time --------------------------------------------------------------------- " + dfw.format(year) + "/" + dfw.format(month) + "/" + dfw.format(day)
            + " " + df2.format(timeDay) + " (UTC)");
        System.out.println("Julian Day (date only) is ----------------------------------------------------- " + julianDayRaw);
    ...
    public static double observer() {
        ...
        double thetaSidereal = THETAZERO + THETAONE * (julianDay - J2K) - longitude; //raw angle, remove multiples of 360   Change to plus longitude for east +ve, west -ve
    ...
    public static double solarTransit() {
        ...
        //(repetition method available to increase accuracy)
        System.out.println("Time of transit (Julian Day) -------------------------------------------------- " + jTransit);
        return jTransit;
    }

    //calculate transit time (hours & minutes)
    public static double solarTransitTime() {
        double jTransitTimeHours = (jTransit - julianDayRaw)*24;
        double jTransitTimeMinutes = (jTransitTimeHours - (Math.floor(jTransitTimeHours)));
        jTransitTimeHours = jTransitTimeHours - jTransitTimeMinutes;
        if (jTransitTimeHours >= 24) {
            jTransitTimeHours = jTransitTimeHours - 24;
        }
        jTransitTimeMinutes = jTransitTimeMinutes*60;
        System.out.println("Time of transit (hours and minutes) ------------------------------------------- " + dfw.format(jTransitTimeHours) + "."
                + dfm.format(jTransitTimeMinutes) + " (UTC)");
        return jTransitTimeHours;
    }

    //calculate sunrise & sunset times
    public static void riseAndSet() {
        //constants for daytime length coefficients (earth, degrees)
        final double H1 = 22.137;
        final double H3 = 0.599;
        final double H5 = 0.016;
        //constants for atmospheric refraction effect and apparent solar disc diameter (earth, degrees)
    ...
    public static void riseAndSetTime() {
        double sunriseTimeHours = (jRise - julianDayRaw)*24;
        double sunriseTimeMinutes = (sunriseTimeHours - (Math.floor(sunriseTimeHours)));
        sunriseTimeHours = sunriseTimeHours - sunriseTimeMinutes;
        if (sunriseTimeHours >= 24) {
            sunriseTimeHours = sunriseTimeHours - 24;
        }
        sunriseTimeMinutes = sunriseTimeMinutes*60;
        System.out.println("Time of sunrise (hours and minutes) ------------------------------------------- " + dfw.format(sunriseTimeHours) + "."
                + dfm.format(sunriseTimeMinutes) + " (UTC)");

        double sunsetTimeHours = (jSet - julianDayRaw)*24;
        double sunsetTimeMinutes = (sunsetTimeHours - (Math.floor(sunsetTimeHours)));
        sunsetTimeHours = sunsetTimeHours - sunsetTimeMinutes;
        if (sunsetTimeHours >= 24) {
            sunsetTimeHours = sunsetTimeHours - 24;
        }
        sunsetTimeMinutes = sunsetTimeMinutes*60;
        System.out.println("Time of sunset (hours and minutes) -------------------------------------------- " + dfw.format(sunsetTimeHours) + "."
                + dfm.format(sunsetTimeMinutes) + " (UTC)");
    }
